...
Every function below is also callable by a short `opt_` alias —
`anofox_optimize_pack_best_of` is `opt_pack_best_of`. The aliases are
registered with `alias_of` and carry no separate documentation, so they
are listed once here rather than twice.
...
## anofox_optimize_portfolio_top_return_optimized

`anofox_optimize_portfolio_top_return_optimized(['DOUBLE[]', 'DOUBLE[]', BIGINT, DOUBLE])` -> `STRUCT(weights DOUBLE[], expected_return DOUBLE, volatility DOUBLE, sharpe DOUBLE)`
...
Example: `anofox_optimize_portfolio_top_return_optimized([0.12,0.10,0.10],[0.04,0.036,-0.01,0.036,0.04,-0.01,-0.01,-0.01,0.04], 2, 0.6)`
